...
module.exports = {
    ...
    callback: async ({ sock, m, command, cmdSuccess, cmdFailed, isImage, isVideo, isDocument, isQuotedDocument, isQuotedImage, isQuotedVideo }) => {
        try {
            const data = (await sock.getAllGroups(true)).filter(({ id, participants, announce }) => (id !== m.chat && Object.keys(db.sewa).includes(id) && participants.filter((x) => x.admin !== null).map((x) => x.id).includes(m.botNumber) && announce || id !== m.chat && Object.keys(db.sewa).includes(id)  && !announce)).map((x) => x.id) 
            if (isImage || isQuotedImage) {
                ...
                for (let x of data) {
                    ...
                    const mention = [...teks.matchAll(/@([0-9]{5,16}|0)/g)].map((x) => x[1]).filter((x) => !isNaN(parseInt(x))).map((x) => x + "@s.whatsapp.net")

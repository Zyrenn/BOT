...
module.exports = {
    ...
    callback: async ({ sock, m, command, cmdSuccess, cmdFailed, isImage, isVideo, isDocument, isQuotedDocument, isQuotedImage, isQuotedVideo }) => {
        try {
            const data = (await sock.getAllGroups(true)).filter(({ id, participants, announce }) => (id !== m.chat && participants.filter((x) => x.admin !== null).map((x) => x.id).includes(m.botNumber) && announce || id !== m.chat && !announce)).map((x) => x.id) 
